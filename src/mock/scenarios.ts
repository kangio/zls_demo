...
export const scenarios: Record<ScenarioKey, Scenario> = {
  rag: {
    key: 'rag', name: '金融投研洞察 Agent', short: '研报检索、财务分析、风险核验与投研摘要闭环', model: 'Qwen3-32B', modelSize: '32B Dense', framework: 'vLLM 0.23', runtime: 'CUDA 13.1', comm: 'NCCL 2.30', gpu: 'A5 64GB', gpuCount: 48, nodes: 6, clusterNodes:[3,3],
    ...
    requestMix: [{label:'研报检索问答',share:46,prefix:'6K',input:'10K',output:'1.1K'}, {label:'财务指标分析',share:34,prefix:'12K',input:'18K',output:'1.8K'}, {label:'风险核验摘要',share:20,prefix:'18K',input:'28K',output:'2.4K'}],
    prefixHotness: {top:'Top 12%',contribution:70,distribution:'Zipf α 1.08'},
    prefixItems: [{label:'投研任务上下文',value:42,meta:'任务内'}, {label:'研报与财务数据',value:38,meta:'标的内'}, {label:'合规 System Prompt',value:20,meta:'全局'}],
    evolutionBurst: '研报证据 +1.2K',
    evolution: [{round:'R1',action:'载入标的资料',tokens:'+2.4K',type:'add'}, {round:'R2',action:'检索研报证据',tokens:'+1.2K',type:'add'}, {round:'R3',action:'财务工具返回',tokens:'+0.8K',type:'add'}, {round:'R4',action:'证据去重压缩',tokens:'-0.7K',type:'trim'}, {round:'R5',action:'生成投研摘要',tokens:'+1.0K',type:'add'}],
    ...
    throughputTarget: 0.55, throughputResult: 0.63, maxQps: 360, maxConcurrency: 640, kvHitRate: 82.5
  },
  long: {
    key: 'long', name: '法律合同审查 Agent', short: '合同解析、条款比对、风险识别与审查意见闭环', model: 'DeepSeek-V3', modelSize: '671B / 37B Active', framework: 'MindIE 2.0', runtime: 'CANN 8.0', comm: 'HCCL 8.0', gpu: 'A5 64GB', gpuCount: 1024, nodes: 128, clusterNodes:[64,64],
    ...
    requestMix: [{label:'合同条款抽取',share:35,prefix:'16K',input:'48K',output:'2K'}, {label:'多合同条款比对',share:45,prefix:'32K',input:'64K',output:'3K'}, {label:'尽调风险综合研判',share:20,prefix:'48K',input:'96K',output:'4K'}],
    prefixHotness: {top:'Top 15%',contribution:68,distribution:'Zipf α 0.96'},
    prefixItems: [{label:'审查会话',value:55,meta:'会话内'}, {label:'合同与法规上下文',value:30,meta:'案件内'}, {label:'审查规则 Prompt',value:15,meta:'全局'}],
    evolutionBurst: '主合同载入 +18K',
    evolution: [{round:'R1',action:'载入主合同',tokens:'+18K',type:'add'}, {round:'R2',action:'追加附件条款',tokens:'+3.6K',type:'add'}, {round:'R3',action:'关联法规证据',tokens:'+2.9K',type:'add'}, {round:'R4',action:'重复条款压缩',tokens:'-8.4K',type:'trim'}, {round:'R5',action:'生成审查意见',tokens:'+4.1K',type:'add'}],
  ...
  coding: {
    key: 'coding', name: '软件研发协作 Agent', short: '任务规划、代码生成、工具执行与验证闭环', model: 'Qwen3-32B + DeepSeek-Coder-V2', modelSize: '32B + 236B / 21B Active', framework: 'vLLM 0.23', runtime: 'CUDA 13.1', comm: 'NCCL 2.30', gpu: 'A5 64GB', gpuCount: 896, nodes: 112, clusterNodes:[40,72],
    ...
    modelInputs: [
      ...
      {
        id:'coder', label:'模型 B', role:'代码生成与仓库级修复', symbol:'DC', model:'DeepSeek-Coder-V2', modelSize:'236B / 21B Active', framework:'vLLM 0.23', runtime:'CUDA 13.1', comm:'NCCL 2.30',
        ...
        requestMix:[{label:'代码定位与理解',share:34,prefix:'12K',input:'24K',output:'2K'},{label:'代码生成与修复',share:48,prefix:'28K',input:'42K',output:'5K'},{label:'仓库级验证',share:18,prefix:'44K',input:'72K',output:'7K'}],
        prefixHotness:{top:'Top 8%',contribution:68,distribution:'Zipf α 1.08'}, prefixItems:[{label:'代码上下文',value:49,meta:'仓库内'},{label:'工具执行结果',value:33,meta:'任务内'},{label:'System Prompt',value:18,meta:'全局'}],
        evolutionBurst:'仓库扫描 +7.6K', evolution:[{round:'R1',action:'扫描仓库',tokens:'+7.6K',type:'add'},{round:'R2',action:'生成补丁',tokens:'+3.1K',type:'add'},{round:'R3',action:'测试反馈',tokens:'+2.4K',type:'retry'},{round:'R4',action:'压缩并验证',tokens:'-1.2K',type:'trim'}]
      }
    ]
  }
}
