...
const initialAgents:AgentConfig[]=[
  {id:'insight',index:'AGENT 01',name:'知识洞察 Agent',role:'检索、归纳与可信回答',icon:'database',color:'cyan',model:'Qwen3-32B',peak:1280,average:620,ttft:600,tpot:45,throughput:'1.3M'},
  {id:'research',index:'AGENT 02',name:'深度研究 Agent',role:'长上下文分析与多步推理',icon:'layers',color:'violet',model:'DeepSeek-V3',peak:860,average:390,ttft:1800,tpot:55,throughput:'1.2M'},
  {id:'coding',index:'AGENT 03',name:'研发执行 Agent',role:'任务规划、工具调用与代码生成',icon:'cube',color:'green',model:'Qwen3-32B',peak:2460,average:1180,ttft:900,tpot:45,throughput:'4.2M'},
]
...
function AgentSettingsDrawer({agent,onClose,onUpdate}:{agent:AgentConfig;onClose:()=>void;onUpdate:(id:string,key:keyof AgentConfig,value:string|number)=>void}){
  ...
  const inferenceStack={framework:'vLLM 0.23',runtime:'CUDA 13.1',comm:'NCCL 2.30'}
    ...
    <div className="simulation-canvas">
      <div className="planning-live-grid"><aside className="dimension-map"><div className="dimension-map-head"><span>7D PARAMETERS</span><b>动态规划参数</b></div>{optimizationDimensions.map((dimension,i)=><div key={dimension[0]} className={`dimension-item dimension-${i+1} ${dimensionProgress[i]===100?'dimension-complete':''}`}><DimensionProgressBorder progress={dimensionProgress[i]}/><i>{i+1}</i><span><b>{dimension[0]}</b><em className="dimension-scope">{dimension[1]}</em><small className="dimension-live-value"><span>{dimensionValues[i]}</span></small></span></div>)}</aside><div className="live-topology">
        ...
        <div className="sim-layer model-layer"><label>02 · MODEL PERFORMANCE & ENGINE TUNING</label><div>
          <span className="sim-model model-0"><Icon name="cube" size={17}/><b>Qwen3-32B</b><small>Agent 01 + Agent 03 · {qwenInstances} Replicas · TP{qwenTp}/PP1</small><div className="model-live-kpis">{kpiBar('TTFT',`${qwenTtft}ms`,28+(qwenTtft-agentPlanNumbers.qwen.ttftResult)/240*62,'latency','P95 · ↓')}{kpiBar('TPOT',`${qwenTpot}ms`,28+(qwenTpot-agentPlanNumbers.qwen.tpotResult)/18*62,'latency','P95 · ↓')}{kpiBar('输出',`${qwenThroughput}M`,28+(Number(qwenThroughput)-4)/2*62,'throughput','TPS · ↑')}</div><div className="model-engine-tunables"><span>Batch <b>{qwenBatch}</b></span><span>Chunk <b>{dimensionProgress[1]<58?'8K':'4K'}</b></span><span>Prefix <b>{qwenCacheHit}%</b></span><span>P/D <b>{qwenPrefillShare}:{100-qwenPrefillShare}</b></span></div></span>
          <span className="sim-model model-1"><Icon name="cube" size={17}/><b>DeepSeek-V3</b><small>Agent 02 · {deepseekInstances} Replicas · TP{deepseekTp}/PP{deepseekPp}/EP{deepseekEp}</small><div className="model-live-kpis">{kpiBar('TTFT',`${dsTtft}ms`,28+(dsTtft-agentPlanNumbers.deepseek.ttftResult)/980*62,'latency','P95 · ↓')}{kpiBar('TPOT',`${dsTpot}ms`,28+(dsTpot-agentPlanNumbers.deepseek.tpotResult)/24*62,'latency','P95 · ↓')}{kpiBar('输出',`${dsThroughput}M`,28+(Number(dsThroughput)-.8)/.6*62,'throughput','TPS · ↑')}</div><div className="model-engine-tunables"><span>Batch <b>{dsBatch}</b></span><span>Chunk <b>{dimensionProgress[1]<62?'16K':'8K'}</b></span><span>Prefix <b>{dsCacheHit}%</b></span><span>P/D <b>{deepseekPrefillShare}:{100-deepseekPrefillShare}</b></span></div></span>
        </div></div>
...
function DetailedSimulationTopology({progress}:{progress:number}){
  ...
  return <div className="sim-network detailed"><label>03 · HOMOGENEOUS LEAF–SPINE FABRIC · SERVER RESOURCE PLACEMENT</label><svg viewBox="0 0 820 565" role="img" aria-label="同构 Leaf Spine 网络以及服务器内部模型实例 NPU HBM HCCS DDR NIC SSD 连接关系">
    ...
    <g className="sim-leaves">{[116,312,508,704].map((x,i)=><g key={x} className="progressive-leaf" style={{opacity:servers[i].nodeCount>0?.35+servers[i].reveal*.65:0}}><rect x={x-55} y="92" width="110" height="27" rx="4"/><text x={x} y="109">L{i+1} GROUP · {servers[i].nodeCount}×400G</text></g>)}</g>
...
const sloGroups=agentPlan?
  [{title:'共享模型服务 · Agent 01 + 03',detail:`TTFT P95 ≤ ${agentPlanNumbers.qwen.ttftTarget} ms · TPOT P95 ≤ ${agentPlanNumbers.qwen.tpotTarget} ms · 输出吞吐 ≥ ${agentPlanNumbers.qwen.throughputTarget.toFixed(1)}M tps`},{title:'独立模型服务 · Agent 02',detail:`TTFT P95 ≤ ${agentPlanNumbers.deepseek.ttftTarget.toLocaleString()} ms · TPOT P95 ≤ ${agentPlanNumbers.deepseek.tpotTarget} ms · 输出吞吐 ≥ ${agentPlanNumbers.deepseek.throughputTarget.toFixed(1)}M tps`}]:
...
function Plan({s,hideTopology=false,agentPlan=false}:{s:(typeof scenarios)[ScenarioKey];hideTopology?:boolean;agentPlan?:boolean}){
  ...
  const kvModelPlans=agentPlan?[
    {model:'Qwen3-32B',scope:'Agent 01 + 03 · 高 Prefix 复用',tone:'qwen',stack:'LMCache · Prefix Cache · Cost-aware LRU',tiers:[['HBM',agentPlanNumbers.qwen.hbm.toFixed(1)],['DDR',agentPlanNumbers.qwen.ddr.toFixed(1)],['Local SSD',agentPlanNumbers.qwen.localSsd.toFixed(1)],['Remote SSD',agentPlanNumbers.qwen.remoteSsd.toFixed(1)]]},
    {model:'DeepSeek-V3',scope:'Agent 02 · 长上下文推理',tone:'deepseek',stack:'LMCache · Mooncake Store · Adaptive Prefetch',tiers:[['HBM',agentPlanNumbers.deepseek.hbm.toFixed(1)],['DDR',agentPlanNumbers.deepseek.ddr.toFixed(1)],['Local SSD',agentPlanNumbers.deepseek.localSsd.toFixed(1)],['Remote SSD',agentPlanNumbers.deepseek.remoteSsd.toFixed(1)]]}
