...
const getDeploymentZones=(s:Scenario):DeploymentZone[]=>{
  if(s.key==='coding')return [
    ...
    {model:'DeepSeek-Coder-V2',modelLabel:'MODEL B',role:'EDGE P/D CELL',units:[{kind:'prefill',nodes:8,instances:4},{kind:'decode',nodes:8,instances:4}],affinity:'热点任务溢出 · 16 Nodes',side:'standard'},
...
function PlanningNetworkTopology({scenario:s}:{scenario:Scenario}){
  ...
  </svg><div className="planning-legend"><span><i className="need"/>输入物理拓扑同源映射</span>{s.key==='coding'&&<><span><i className="model-a"/>模型 A</span><span><i className="model-b"/>模型 B</span></>}<span><i className="flow"/>模型 / P-D 数据流</span><span><i className="hot"/>橙色 = 峰值窗口 P95 最高资源</span></div><div className="fabric-demand">Fabric 加权平均 {fabricDemandTbps.toFixed(2)} / 1.60 Tbps · {profile.fabric}% · 峰值窗口 P95 {profile.fabricP95}%</div></div>
}
