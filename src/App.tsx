    ...
    <div className="simulation-canvas">
      <div className="planning-live-grid"><aside className="dimension-map"><div className="dimension-map-head"><span>7D PARAMETERS</span><b>动态规划参数</b></div>{optimizationDimensions.map((dimension,i)=><div key={dimension[0]} className={`dimension-item dimension-${i+1}`}><i>{i+1}</i><span><b>{dimension[0]}</b><em className="dimension-scope">{dimension[1]}</em><small>{dimensionValues[i]}</small></span></div>)}</aside><div className="live-topology">
        ...
        <div className="sim-layer model-layer"><label>02 · MODEL PERFORMANCE & ENGINE TUNING</label><div>
          <span className="sim-model model-0"><Icon name="cube" size={17}/><b>Qwen3-32B</b><small>Agent 01 + Agent 03 · {qwenInstances} Instances · TP8</small><div className="model-live-kpis">{kpiBar('TTFT',`${qwenTtft}ms`,28+(qwenTtft-430)/290*62,'latency','P95 · ↓')}{kpiBar('TPOT',`${qwenTpot}ms`,28+(qwenTpot-31)/16*62,'latency','P95 · ↓')}{kpiBar('吞吐',`${qwenThroughput}M`,28+(Number(qwenThroughput)-7.2)/2.6*62,'throughput','TOK/S · ↑')}</div><div className="model-engine-tunables"><span>Batch <b>{96+Math.round(progress*.32)}</b></span><span>Chunk <b>{progress<52?'8K':'4K'}</b></span><span>Prefix <b>{kvHit}%</b></span><span>P/D <b>{52+Math.round(progress*.12)}:{48-Math.round(progress*.12)}</b></span></div></span>
          <span className="sim-model model-1"><Icon name="cube" size={17}/><b>DeepSeek-V3</b><small>Agent 02 · {deepseekInstances} Instances · EP8</small><div className="model-live-kpis">{kpiBar('TTFT',`${dsTtft}ms`,28+(dsTtft-780)/700*62,'latency','P95 · ↓')}{kpiBar('TPOT',`${dsTpot}ms`,28+(dsTpot-41)/28*62,'latency','P95 · ↓')}{kpiBar('吞吐',`${dsThroughput}M`,28+(Number(dsThroughput)-11.2)/5.4*62,'throughput','TOK/S · ↑')}</div><div className="model-engine-tunables"><span>Batch <b>{48+Math.round(progress*.24)}</b></span><span>Chunk <b>{progress<58?'16K':'8K'}</b></span><span>Prefix <b>{Math.max(38,kvHit-11)}%</b></span><span>P/D <b>{56+Math.round(progress*.08)}:{44-Math.round(progress*.08)}</b></span></div></span>
        </div></div>
